...
int mpu_init()//(struct int_param_s *int_param)
{
    ...
    while(CT_TICKS_SINCE(count) < MS_TO_CORE_TICKS(100)) {};
...
#define MPU6500_MEM_REV_ADDR    (0x17)
    ...
    if (rev == 0x1)
        st.chip_cfg.accel_half = 0;
    else if (rev == 0x3) // Foi adicionado por mim, nao consegui confirmaçao deste facto
        st.chip_cfg.accel_half = 0;
    else {
        return -1;
    }
   ...
   // Specific for msp430 microcontroller
//    if (int_param)
...
int mpu_reset_fifo(void)
{
    ...
    if (st.chip_cfg.dmp_on) {
        ...
        /* Delay 50ms*/
        UINT32 count = ReadCoreTimer();
        while(CT_TICKS_SINCE(count) < MS_TO_CORE_TICKS(50)) {};
...
int mpu_set_bypass(unsigned char bypass_on)
{
    ...
    if (bypass_on) {
        ...
        /* Delay 3ms*/
        UINT32 count = ReadCoreTimer();
        while(CT_TICKS_SINCE(count) < MS_TO_CORE_TICKS(3)) {};
...
static int compass_self_test(void)
{
    ...
    do {
        ...
        /* Delay 10ms*/
        UINT32 count = ReadCoreTimer();
        while(CT_TICKS_SINCE(count) < MS_TO_CORE_TICKS(10)) {};
...
static int get_st_biases(long *gyro, long *accel, unsigned char hw_test)
{
    ...
    /* Delay 200ms*/
    UINT32 count = ReadCoreTimer();
    while(CT_TICKS_SINCE(count) < MS_TO_CORE_TICKS(200)) {};
    ...
    /* Delay 15ms*/
    count = ReadCoreTimer();
    while(CT_TICKS_SINCE(count) < MS_TO_CORE_TICKS(15)) {};
    ...
    /* Delay test.wait_ms ms*/
    count = ReadCoreTimer();
    while(CT_TICKS_SINCE(count) < MS_TO_CORE_TICKS(test.wait_ms)) {};
...
static int setup_compass(void)
{
    ...
    /* Delay 1 ms*/
    UINT32 count = ReadCoreTimer();
    while(CT_TICKS_SINCE(count) < MS_TO_CORE_TICKS(1)) {};
